...
Google requires creating a project under the Google Cloud Compute console. 
1. Create a project, generate a service key, and download to the google_keys dir. 
2. Modify the variable.tf and main.tf files to reference these files to reflect your service account key json file and your project-id. 
3. Google also requires setting up firewall rules: Settings -> VPC network -> Firewall rules. Create a new rule: network=default, priority=1, direction=ingress, action on match=allow, ip-range=0.0.0.0/24, protocols:ports=tcp:22,tcp:80,tcp:443
